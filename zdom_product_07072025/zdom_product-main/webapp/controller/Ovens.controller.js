sap.ui.define(
  ...
  function (
    ...
    return Controller.extend("production.controller.Ovens", {
      ...
      loadPopUp: function (oData) {
        ...
        if (
          ...
          if (oData.ProductionDeclarationSet.results.length == 1) {
            ...
            inpGamng = oData.ProductionDeclarationSet.results[0].Gamng;
            inpCantRest = oData.ProductionDeclarationSet.results[0].CantRest;
            ...
            .create("/HeaderSet", oPostProduct, {
              success: function (oData) {
                ...
                if (oData.ReturnSet.results.length == 0 || oData.ReturnSet.results[0].Type == 'S') {
                  ...
                  oFilters.push(new sap.ui.model.Filter("Werks", FilterOperator.EQ, aData.Plant));
                  oFilters.push(new sap.ui.model.Filter("WorkCtr", FilterOperator.EQ, aData.WorkCenter));

                  this.getEntitySet(sPath, oFilters).then(function (oData) {
                    me.onCreateModelOvens(oData);
                    busyDialog4.close();
      ...
      getEntitySet: function (sPath, aFilter) {
        if (aFilter.length < 0) {
          var aFilters = [];
        } else {
        ...
        return new Promise(function (resolve, reject) {
          this.getView().getModel("ZDOM_0000_SRV_01").read(sPath, {
            filters: aFilters,
            success: resolve,
            error: reject
          });
